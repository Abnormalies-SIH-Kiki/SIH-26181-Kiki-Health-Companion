...
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg)
{
    ...
    esp_lv_adapter_display_config_t disp_cfg = {
        ...
        .profile = {
            ...
            /* LOCAL CHANGE to a vendored component (Waveshare BSP v3.0.1).
               Upstream is buffer_height=50 with use_psram=true, which puts the
               LVGL draw buffer in PSRAM -- so spi_master must allocate a
               DMA-capable *internal* bounce buffer of 466*50*2 = 46,600 bytes
               on every single flush. Once TLS and Wi-Fi traffic fragment
               internal RAM, a contiguous 46 KB block stops being available and
               the panel dies permanently with:
                 setup_dma_priv_buffer: Failed to allocate priv TX buffer
                 esp_lvgl:bridge_v9: Draw bitmap failed: ESP_ERR_NO_MEM
               Observed on the remote (wss) link, where TLS is the extra
               pressure; the LAN build never showed it.
               Internal + smaller trades ~30 KB of internal RAM, allocated once
               at boot while memory is still unfragmented, for zero per-flush
               allocation. A fixed cost beats a recurring gamble. */
            .buffer_height = 16,
            .use_psram = false,
