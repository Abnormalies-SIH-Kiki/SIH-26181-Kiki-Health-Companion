...
This is NOT the speaking path and NOT the slow reasoning router
(`generate_llm_resp`). It exists for one job: drive a multi-turn agent loop
fast enough that a complex spoken request finishes inside ~10 seconds.

Design notes (all measured on the Pi via tests/bench_action_agent.py):
...
def _call_cerebras(prompt: str, cfg: dict) -> str:
    ...
    body = {
        ...
        "messages": [{"role": "user", "content": _trim_prompt(
            prompt, int(sub.get("max_prompt_chars", 0)))}],
...
def complete(prompt: str, provider: Optional[str] = None,
    ...
    Tries the configured provider, then the fallback provider. Raises
    ...
    for name in order:
        ...
        try:
            text = _PROVIDERS[name](prompt, cfg)
