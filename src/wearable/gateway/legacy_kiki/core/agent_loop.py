    ...
    for turn in range(max_turns):
        ...
        if parsed is None:
            ...
            if min_tool_calls > 0 and total_tool_calls < min_tool_calls:
                print(f"[{label}] Model returned plain text without making required tool calls. Forcing tool call.")
                conversation.append("SYSTEM ERROR:\nThis focus/task requires active research/verification using tools. You must use tools (like search_web) to find real facts first. Please output a JSON tool_calls command.")
                continue
        ...
        if parsed.get("status") == "completed":
            if min_tool_calls > 0 and total_tool_calls < min_tool_calls:
                print(f"[{label}] Model tried to complete without making required tool calls. Forcing tool call.")
                conversation.append("SYSTEM ERROR:\nThis focus/task requires active research/verification using tools. You cannot complete the task yet because you have not executed any tools. Please execute the appropriate tool call(s) (e.g. search_web or others) to gather facts first.")
                continue
        ...
        if parsed.get("status") == "failed":
            if min_tool_calls > 0 and total_tool_calls < min_tool_calls:
                print(f"[{label}] Model tried to fail without making required tool calls. Forcing tool call.")
                conversation.append("SYSTEM ERROR:\nThis focus/task requires active research/verification using tools. You cannot fail the task yet because you have not executed any tools. Please execute the appropriate tool call(s) (e.g. search_web or others) to gather facts first.")
                continue
        ...
        if not tool_calls:
            if min_tool_calls > 0 and total_tool_calls < min_tool_calls:
                print(f"[{label}] Model tried to finish without making required tool calls. Forcing tool call.")
                conversation.append("SYSTEM ERROR:\nThis focus/task requires active research/verification using tools. You cannot complete the task yet because you have not executed any tools. Please execute the appropriate tool call(s) (e.g. search_web or others) to gather facts first.")
                continue
