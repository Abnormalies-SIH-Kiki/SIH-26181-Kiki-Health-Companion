...
Always run  source /home/vaibhav/KikiESP32/.venv/bin/activate before main.py to get into the venv..
---
...
| Foreground speaking LLM (gemma-4-26B-A4B, llama-server) | Vaibhav's laptop (RTX 4060) over Tailscale | `http://127.0.0.1:8080/v1` |
| TTS (omnivoice.cpp tts-server, 24 kHz PCM) | Same laptop | `http://127.0.0.1:8082` |
| STT (whisper.cpp server, HTTP POST + Silero VAD) | Same laptop (Tailscale) | `http://127.0.0.1:5555/inference` |
| Cloud LLM fallback | Vertex AI Gemini (via litellm) / google-genai / Groq | API keys in `.env` + an optional Vertex service-account key at `VERTEX_SERVICE_ACCOUNT_FILE` |
...
name** — every DM is a bare identifier like `236601208778873`. So "send a message to
...
them turns `128668764475550` into "Nikhil" — verified 8/8 on live group senders.
...
- `jid_variants(jid)` → **the phone-vs-`@lid` split**: a direct chat is *addressed* by
  phone but *filed* under its `@lid`. "Moshe Website" resolves to
  `16479932193@s.whatsapp.net` (0 message rows) while its 60 real messages live under
  `23145595560020@lid` — querying only the first made the agent state "there are no
...
Measured after: `namitha`→Namita, `moshe graty website`→Moshe Website, `burrito time`→the
burgito group, `bharat`→Bharat Pandey; `moshe` alone stays ambiguous (three real Moshes),
...
Verified: with "moshe website has been messaging me all week" in history, **"summarize my
chat with him"** picks Moshe Website out of three Moshes — 9.4s, 974 chars.
...
- **`tools.py` (1678)** — all tools. Layout:
  ...
  - tool impls @174–1052 (all async): `search_web` (Exa, 3 results, highlights, IN locale,
    ...
    (subprocess with the `/home/vaibhav/KikiESP32/.venv/bin/python` venv), five compact
...
`source /home/vaibhav/KikiESP32/.venv/bin/activate && python tests/calibrate_lcd_sync.py`.
