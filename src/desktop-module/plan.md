...
## Wearable + SIH health implementation (2026-09-01; implemented, real-device lock pending)

2026-09-06 physical fall attempt: logs at 19:36:06 report "fall candidate expired:
no sustained stillness" amid repeated shake/rocking events. The raw detector did
arm, but the eight-second stillness gate was not satisfied. Shaker speech is an
independent personality reaction. This is NOT successful fall acceptance: the
heuristic can miss falls followed by continued motion. User advised not to test
by falling personally; use replay/controlled padded fixtures, with no unannounced
caregiver escalation. Watch-style UI redesign uses four coloured native LVGL
cards, a smaller mascot and a large clock; no extra weather network/LLM work.

2026-09-06 follow-up: user confirmed repaired voice works well. Idle AMOLED
dashboard and raw-IMU fall repair are deployed: build `a045401d` was USB flashed,
hash-verified, and reported its LAN hello at 19:31:36. Dashboard uses
the RPi's existing cached environment provider, exposed through a read-only
health-service endpoint (deployed and verified), not a new weather fetcher or
LLM injection. It labels HR age, stale/missing weather, and steps since boot.
Fall detection no longer depends on motion-animation event delivery or waits
behind optical acquisition. It still requires recent wear and the configured
freefall/impact/stillness ladder; controlled hardware acceptance is outstanding.
310 gateway tests, 12 RPi health-service tests and host fall-detector checks pass.
Web UI chat/context recording repair is deployed; its restart-safe event cursor
was verified live. All 310 gateway tests also pass on the laptop. The board's
new microphone stream resumed at 1.03x; physical dashboard/fall acceptance remains
pending. No `gateway/legacy_kiki` source files changed.

2026-09-06 context-overflow repair: GPT-token estimates and a raised compaction
threshold let the local model reject prompts over its 8192-token window. Cloud
summary work was also cancelled with foreground turns. The ESP gateway now counts
the normalized model template using the actual tokenizer, reserves reply capacity,
and stages core-owned cloud summaries for safe application between turns. Emergency
trimming archives full transcripts and preserves the latest request. Empty model
results receive one retry and a spoken failure if still empty. Previous summaries
are replaceable memory rows. 302 automated gateway tests pass on both machines;
long physical conversations remain an acceptance check.

2026-09-06 voice regression: laptop logs confirmed health HTTP connection
timeouts were blocking the ESP32 gateway's single receive loop, causing reconnects
and delayed microphone/Stop events. Repair moves delivery into a bounded,
deduplicated background queue and starts the follow-up reply window at actual
speaker completion. Health-service outages must not interrupt voice; this is an
explicit acceptance requirement. Firmware and `gateway/legacy_kiki` stay unchanged
for this repair. Automated regression coverage includes stalled delivery, Stop,
ping, playback completion, retry bounds and cancellation. All 292 gateway tests
pass on Pi and laptop; the fix was deployed at 00:45. Physical Stop worked at
00:46:40 (gateway turn cancelled; ESP32 playback buffer drained). Natural spoken
follow-up after completed playback remains a physical acceptance check.

- [x] MAX30102 wrist sensing on ESP32 GPIO18/17 with stillness-gated 40-second windows.
- [x] Steps, wear/activity state, experimental uncalibrated SpO2, and fall ladder.
- [x] Three-batch reboot-safe store-forward queue and explicit Pi acknowledgement.
- [x] Dedicated Pi health service/API/dashboard on port 8091 using `care_plan.json`.
- [x] Cross-process locked/merged care-plan persistence for Kiki + health service.
- [x] Desktop Kiki wearable context in its existing cache-safe `Now/Power/Body` row.
- [x] Pi Kiki wearable context through the existing change-gated `CARE NOW` row.
- [x] Email + WhatsApp fall notification attempts with accepted/delivered distinction.
- [x] On-device Settings measurement UI with in-place retry and an uninterrupted 10-second capture.
- [x] Firmware and Pi use peak intervals as the BPM source and independently verify either
  peak/ACF consensus or an explicit, well-supported paired-peak notch correction; weak ACF,
  recovered I2C faults, saturation, and implausible windows cannot enter desktop context.
- [ ] Lock on real wearable: calibrate contact gate, step accuracy, HR signal quality,
  fall cancellation/escalation, service reachability, and caregiver delivery receipts.

Real-device debugging on 2026-09-03 found and fixed a `kiki_health` task stack overflow
(old coredump: 6272 bytes used, 136 free), then exposed an intermittent marginal I2C bus.
Build `5f3656f8` adds a 12 KiB task stack, PSRAM analysis scratch, 25 kHz sensor bus,
complete retry/overflow accounting, saturation rejection, and Pi-side defense in depth.
The follow-up build also gives a full 192-byte FIFO drain its required 120 ms budget;
the old 30 ms limit caused the observed clean-then-permanently-busy cascade whenever the
32-sample FIFO filled.
Flashed build `46b2c738` removes the mid-capture analysis pause and prevents low-confidence
ACF values (observed scores 0.047–0.241) from overriding the proven pulse-interval result; a
single ten-second window is analysed once. The invalid validation rows were removed from trusted history/context. A clean
quality-approved physical measurement on this build remains the lock gate.
...
each phase, but mark a phase **locked** only after automated tests pass, Vaibhav tests the
...
- Work proceeds one phase at a time. At each gate, Codex gives Vaibhav exact spoken actions;
  Vaibhav interacts with Kiki; then logs, persisted state, UI state, and delivery receipts are
...
- [ ] Remove or migrate obsolete test routines and the old hydration event from the live care
  plan after Vaibhav approves cleanup.
...
- [x] **The visual verdict is no longer written after the praise** (2026-09-02). The 19:49 neck
  session confirmed six consecutive asanas — "held the position steadily across both frames",
  turn after turn — while nobody was moving; each `visual_observation` simply restated the
  instruction just given, in the prompt's own wording. Three fixes: `visual_observation` and a new
  structured `instruction_followed` verdict are now emitted BEFORE `summary`, so grounding
  precedes judgement; anything but `"yes"` deterministically forces `incorrect_form` and a retry;
  and the hold-frame prompt text no longer supplies "reached, held steady, released" for the model
  to hand back. A vision-enabled routine that loses the camera now says so and hands the turn back
  instead of advancing blind, and byte-identical frames on two consecutive turns are treated as a
  frozen camera rather than as evidence.
- [x] **A wrong movement is corrected mid-routine; only a second one in a row stops it**
  (2026-09-02). Stopping on every mismatch made the routine an interview, and `main.py` could open
  Listening on a turn where nothing had been spoken at all — the person was asked to answer a
  question they never heard. First mismatch: say what was wrong, repeat the same movement, mic
  stays muted. Second in a row, an empty frame, or no frame at all: stop and ask.
  `guided_exercise.violations_before_listening` (2) and `retry_hold_seconds` (8) are configurable.
...
- [ ] Rehearse a full day-in-the-life with Vaibhav as the senior and a phone as caregiver, in
  `health_sih` mode.
...
| 2026-09-02 | guided-vision grounding repair | Care agent visual verdict | Reproduced from the 19:49:55–19:51:59 neck transcript: seven turns, six of them confirming an asana from the instruction rather than the frames. `visual_observation`/`instruction_followed` moved ahead of `summary`, negative verdicts force a retry in code, blind and frozen-camera turns hand back to the person. 17 new regressions; full suite 1076 passed / 0 failed. | Needs one real guided neck session where the person deliberately does nothing, and one where they leave the frame |
| 2026-08-31 | sourced-context routing repair | Unified Idle Mind -> foreground hand-off | Replayed the exact "what's going on" -> "I mean in my WhatsApp" failure. Notes now retain source/reason, fresh read-only notes answer deterministically, stale/action/source-mismatch requests route live, and split-turn clarification reaches `complex_query` with both verbatim user fragments. 10 new regressions; focused 279 passed; full suite 956 passed / same 5 pre-existing startup-GPIO failures. | Implementation proven in-process; one short real-voice confirmation remains before lock |
