...
async def conversation_topics(kind: str = "", limit: int = 12) -> str:
    """What Kiki actually knows about this person, as things to talk about.

    `recall_memory` answers a question you already have. This answers the
    question an engagement session starts with -- "what is there to talk about
    at all?" -- which is the difference between opening with something from
    someone's own life and opening with a generic prompt.

    Names only, not content: the list is material to CHOOSE from, and pulling
    the full text of a dozen memories to pick one would be most of a prompt
    spent on eleven things that get discarded. Follow up with `recall_memory`
    on whichever one is worth using.

    kind: people | topics | recent | empty for a mix.
    """
    try:
        ...
        path = Path(configured) if configured else root / "state" / "knowledge_base.json"
    ...
    if kind in ("", "people"):
        ...
        if isinstance(people, dict):
            # `guest_...` entries are unrecognised faces, not people with a
            # history worth reminiscing about.
            out["people"] = [n for n in people
                             if not str(n).lower().startswith("guest_")][:limit]
    if kind in ("", "topics"):
        names = []
        for section in ("facts", "learnings"):
            value = data.get(section)
            if isinstance(value, dict):
                names.extend(value.keys())
        out["topics"] = names[-limit:]
    if kind in ("", "recent"):
        events = []
        value = data.get("experiences")
        if isinstance(value, list):
            for row in value[-limit:]:
                if isinstance(row, dict) and row.get("event"):
                    events.append(f"{row.get('event')} ({str(row.get('date',''))[:10]})")
        out["recent_experiences"] = events

    try:
        from core.brain.thinking_journal import get_journal
        out["background_research"] = [
            str(x)[:90] for x in str(
                get_journal().recent_summaries(6) or "").splitlines() if x.strip()
        ][:6]
    except Exception:
        pass

    if not any(out.values()):
        return "No stored topics yet."
...
async def set_next_turn_note(text: str = "", reason: str = "",
    ...
    return set_note(text, reason, expires_hours, action, source)
...
async def switch_voice(voice: str = "") -> str:
    ...
    if requested in available:
        ...
        # loose match: substring either way handles "amitabh bachchan" etc.
...
async def execute_python_code(code: str) -> str:
    ...
    try:
        ...
        result = await asyncio.get_running_loop().run_in_executor(
            None,
            lambda: subprocess.run(
                ["/home/kiki/Kiki/kiki/bin/python", "-c", code],
                capture_output=True,
                text=True,
                timeout=30,
...
async def list_chats(query: str = "", limit: int = 20, page: int = 0,
    ...
    # The MCP's own search is a substring LIKE, so a misheard name ("burrito")
    # finds nothing for the group really called "burgito time" and the agent
    ...
    *_WHATSAPP_TOOL_SCHEMAS,
    {
        ...
        "function": {
            ...
            "description": "Execute a shell command. Useful for checking system status like temperature, disk space, etc.Install all python packagaes in  /home/kiki/Kiki/kiki/bin/python venv.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Things this person actually has history with, as a list of names to choose from (people, topics, recent experiences). Use to start or steer an engagement session; then recall_memory for the details of the one you picked.",
            "parameters": {
                "type": "object",
                "properties": {
                    "kind": {"type": "string", "description": "people | topics | recent | empty for a mix"},
                    "limit": {"type": "integer"}
                }
            }
        }
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Set, replace, or clear Kiki's single voice-ready next-conversation update, preserving its evidence source.",
            "parameters": {
                "type": "object",
                "properties": {
                    "text": {"type": "string"},
                    ...
                    "source": {
                        "type": "string",
                        "enum": [
                            "whatsapp", "email", "notion", "web",
                            "conversation", "ambient", "vision", "care",
                            "calendar", "memory", "mixed", "unknown"
                        ]
                    },
    ...
    },
    {
        ...
        "function": {
            "name": "switch_voice",
            "description": "Switch Kiki's speaking voice to a named voice (e.g. amitabh, modi, vaibhav, ansh, normal). Use 'default' to reset.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Execute Python code in a subprocess and return the output. Useful for calculations, data processing, file operations, or any task that benefits from code execution.All python code in run in the /home/kiki/Kiki/kiki/bin/python venv.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Start a care routine IMMEDIATELY (guided exercise, reminder, or a mind-engaging session) instead of scheduling it. Use when the person wants to do it now — 'let's do my neck exercise', 'chalo kasrat karte hain' — and ALSO when they are bored, at a loose end, or want company: pass routine='Something to think about'. Do NOT use update_care_plan for this; that only schedules for later. Kiki takes over and guides the session itself, so say nothing about the steps yourself.",
            "parameters": {
                "type": "object",
                "properties": {
