...
human name** — every DM is a number like ``20000000000051``. So "send a
...
Chaining those two turns ``20000000000031`` into "Nikhil" (verified 8/8 on
...
# Names are noisy: WhatsApp push names carry decoration like "~Deepak~".
...
def jid_variants(jid: str) -> list[str]:
    ...
    its history under the opaque ``@lid``. Measured: "Studio Website" resolves to
    ``15550000021@s.whatsapp.net`` (0 message rows) while its 60 real messages
    live under ``20000000000021@lid``. Reading the wrong one returns nothing,
