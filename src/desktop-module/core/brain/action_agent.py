...
AGENT_TOOLS = (
    ...
    # Senior care: the complex agent is the only foreground writer. It reads
    # the whole routine, writes canonical session briefs, then verifies
    # the materialized worker receipt before reporting success.
    "get_care_plan", "update_care_plan", "get_care_schedule_status",
    "start_care_session", "heart_rate_measurement", "alert_family",
    # Escape hatch for Gmail/Notion WRITES and any other connected MCP
    "self_extend_tool_call",
    "execute_python_code",
)

# Offered by name when the agent tries to answer without verifying anything.
# `min_tool_calls` is what stops a fabricated answer, but a rejection is only
# actionable if it names a tool the model can actually reach for on THIS task.
_CARE_VERIFY_TOOLS = ("get_care_plan", "get_care_schedule_status",
                      "update_care_plan", "heart_rate_measurement",
                      "recall_memory", "get_current_time")
_GENERAL_VERIFY_TOOLS = ("search_web", "list_messages", "search_contacts",
                         "read_gmail", "recall_memory", "get_current_time")
...
_DEFAULTS = {
    ...
    "health_measurement_deadline_seconds": 75.0,
...
# Words that mean care wherever they appear.
_CARE_STRONG_RE = re.compile(
    r"\b(?:care\s*(?:plan|routine|session|task|event)|daily\s*routine|medicine|medication|"
    r"medicines?|medications?|pills?|physio|wellbeing|check[- ]?in|appointments?|"
    r"hydration|exercises?|caregiver|senior|heart[_\s-]*rate|pulse|bpm)\b|"
    r"(?:केयर\s*प्लान|रूटीन|दिनचर्या|दवाई|दवा|गोली|व्यायाम|कसरत|योग|"
    r"एक्सरसाइज़|एक्सरसाइज|अपॉइंटमेंट|देखभाल|दिल\s*की\s*धड़कन|नब्ज़|नब्ज|"
    r"बीपीएम|हार्ट\s*रेट)",
    re.IGNORECASE,
)

# Everyday nouns that only mean care when the person is talking about THEIR
# OWN day. Bare-matching these routed ordinary conversation into the care agent
# and its direct-to-TTS path: "play a walking song" became a care request, and
# "what did they say about lunch?" — a WhatsApp question — became one too, which
# is exactly the request the taco-night fabrication guard exists for.
_CARE_LIFESTYLE_RE = re.compile(
    r"\b(?:routine|water|meal|breakfast|lunch|dinner|sleep|walk|walking)\b|"
    r"(?:पानी|खाना|नाश्ता|टहल|नींद|सोना|उठना)",
    re.IGNORECASE,
)
_FIRST_PERSON_RE = re.compile(
    r"\b(?:my|me|mine|myself|i|i'?m|i've)\b|"
    r"(?:मेरा|मेरी|मेरे|मुझे|मुझको|मैंने|मैं|अपना|अपनी|अपने)",
    re.IGNORECASE,
)


# An outbound message that MENTIONS a care topic is a messaging action, not a
# care action. Observed live on 2026-09-01: "respond to Suyash about his work
# ... and list the healthcare features I currently have (AQI monitoring, heart
# rate monitoring, ...)" matched `heart rate`, so the whole WhatsApp send was
# treated as a care-plan mutation. When it stopped to ask which of the two
# Suyashes was meant, that question was thrown away and Kiki said "That care
# action did not complete" instead.
_OUTBOUND_RE = re.compile(
    r"\b(?:repl(?:y|ies)|respond|write\s+back|send|message|msg|text|whats?app|"
    # Only the SPEAKER as recipient is excluded ("send me my heart rate" is a
    # care read). "message my brother" is still outbound -- a different person.
    r"e-?mail|forward|share)\b(?!\s+(?:me|myself)\b)",
    re.IGNORECASE,
)


def _is_outbound_message(request: str) -> bool:
    """True when the request's own verb is "send something to someone else".

    Decided by POSITION, because a request usually leads with what it is
    actually asking for. "respond to Suyash about my heart rate" is a message;
    "schedule my medicine and message my daughter" is still a care write, and
    the care noun coming first is what says so.
    """
    text = str(request or "")
    outbound = _OUTBOUND_RE.search(text)
    if not outbound:
        return False
    care = _CARE_STRONG_RE.search(text) or _CARE_LIFESTYLE_RE.search(text)
    return care is None or outbound.start() < care.start()


def is_care_request(request: str, context: str = "") -> bool:
    text = f"{request or ''}\n{context or ''}"
    if "CARE_PLAN_DELEGATION" in text:
        return True
    # An explicit delegation above still wins; everything else defers to what
    # the request is actually asking to DO.
    if _is_outbound_message(request):
        return False
    if _CARE_STRONG_RE.search(text):
        return True
    return bool(_CARE_LIFESTYLE_RE.search(text)
                and _FIRST_PERSON_RE.search(str(request or "")))


def _active_care_session() -> dict:
    try:
...
# Anything that could CHANGE the plan, run a session step, or take a reading.
# Checked first and wins ties: mistaking a write for a question is what lets a
# model say "done, scheduled for 8" without having scheduled anything.
_CARE_WRITE_RE = re.compile(
    r"\b(?:add|set|schedule|remind|change|update|edit|remove|delete|cancel|"
    r"create|make|start|stop|skip|measure|check\s+my|take\s+my)\b|"
    r"(?:लगा|बदल|हटा|जोड़|सेट|शुरू|बना|रोक|नाप|माप|याद\s*दिला)",
    re.IGNORECASE,
)
# Read-shaped openings. The escape hatch is allow-listed, not block-listed:
# when we cannot positively tell that a request is a question, we keep the old
# hard-failure behaviour rather than risk speaking an unverified claim.
_CARE_READ_RE = re.compile(
    r"\?|^\s*(?:what|when|which|how|did|do|does|is|are|was|were|can|could|"
    r"tell\s+me|show\s+me|list|read)\b|"
    r"(?:क्या|कब|कैसे|कितन|कौन|बताओ|बता\s*दो|दिखाओ|पढ़)",
    re.IGNORECASE,
)


def is_read_only_care_request(request: str, context: str = "") -> bool:
    """True only for a care request that plainly just asks for information.

    Gates the final-turn `allow_unverified_finish` escape. A question answered
    without a tool call is at worst a stale answer; a WRITE reported without a
    tool call is a lie about something that never happened, so writes, guided
    session turns and scheduled routines never get the escape.
    """
    text = f"{request or ''}\n{context or ''}"
    if "CARE_PLAN_DELEGATION" in text:
        return False
    if _CARE_WRITE_RE.search(text):
        return False
    return bool(_CARE_READ_RE.search(str(request or "").strip()))


def _is_heart_rate_flow(request: str, context: str, session: dict) -> bool:
    text = f"{request or ''}\n{context or ''}"
    if re.search(
            r"heart[_\s-]*rate|pulse|\bbpm\b|दिल\s*की\s*धड़कन|नब्ज़|नब्ज|बीपीएम|हार्ट\s*रेट",
            text, re.IGNORECASE):
        return True
    event = session.get("event") or {}
    event_text = "\n".join((str(event.get("title", "")),
                              str(event.get("objective", "")),
                              str(event.get("session_brief", ""))))
    return bool(re.search(
        r"heart[_\s-]*rate|pulse|\bbpm\b|दिल\s*की\s*धड़कन|नब्ज़|नब्ज|बीपीएम|हार्ट\s*रेट",
        event_text, re.IGNORECASE))


def _care_instructions(request: str, context: str) -> str:
    if not is_care_request(request, context) and not _active_care_session():
        return ""
    return """
CARE-PLAN WORKFLOW — THIS OVERRIDES GENERIC REMINDER INSTRUCTIONS:
- Use get_care_plan before reading or changing care. When formulating or
  adapting a routine, read the COMPLETE plan (omit `section`); checking only
  `exercises` or `reminders` hides the person's profile and care context.
  The canonical scheduled
  unit is section="routine_event": title, objective, category, schedule,
  session_brief, continuous_vision, source/evidence, and adaptation metadata.
- session_brief is a substantive natural-language hand-off to a capable voice
  model, not dialogue, an action DSL, or a tiny checklist. Preserve all
  relevant person-specific facts and caregiver material supplied in the
  conversation. The runtime model receives the complete care-plan snapshot and
  independently conducts the session.
- Before saving a plan, reason whether the available context is sufficient to
  make the requested care genuinely individualized, useful, and responsible.
  If materially relevant information is absent or ambiguous, ask for it in a
  natural follow-up instead of manufacturing a shallow generic plan. Decide
  what is relevant from the request; do not run a fixed questionnaire.
- A request for missing HUMAN context is a valid conversational outcome, not a
  tool failure: after inspecting the plan, return status="completed" with
  `summary` equal to the exact natural question Kiki should speak, and do not
  write the event yet. Reserve status="failed" for actual tool/system failure.
- Do not invent diagnoses, prescriptions, medicine/dose changes, or claim that
  generated guidance came from a clinician. Clearly retain any boundaries or
  professional instructions the person/caregiver supplied.
- continuous_vision means a fresh camera frame is visually adjudicated for
  every live conversational turn. Choose it when visual evidence materially
  helps the requested session, not from a hardcoded category table.
- "start it now", "let's do my exercise", "chalo kasrat karte hain" means
  start_care_session(routine="neck"|"waist"|...), NOT update_care_plan.
  update_care_plan only SCHEDULES something for later and has no start action;
  inventing one (action="start_session") just fails and loops. If the routine
  does not exist yet, create it with update_care_plan first, then start it.
- Starting a care_session is only a hand-off. Never simulate the person's
  replies, advance steps, or conduct the session inside the plan-management
  loop. The foreground microphone→care-model→TTS loop owns the conversation.
  Once start_care_session succeeds, say ONE short line at most ("chaliye shuru
  karte hain") — Kiki itself guides every step from there, so do not describe
  the exercise or list its steps.
- Human ambiguity becomes a clarification question. Machine-format errors are
  not human ambiguity: use the validator's message, correct the JSON, and retry.
- NEVER use schedule_worker or set_timer for care. After every routine_event
  add/edit, verify its real worker with get_care_schedule_status and only claim
  it is active when scheduled=true with an exact next_trigger_at.
- source="idle_mind" requires concrete repeated-routine evidence; one
  observation is not a learned routine. Never infer medication or dosage.
- Sensor tools return data and never speak. Only trusted heart-rate readings
  may be presented or trended; this MAX30102 assembly's SpO2 is uncalibrated
  and must not be presented as a health reading.
- Preserve the person's language. Care summaries go directly to TTS, so write
  exactly the natural words Kiki should say—never relay instructions.
"""
...
- Gmail  → self_extend_tool_call(connection="gmail", tool="Gmail_WriteDraftEmail",
  args_json='{"recipient":"...","subject":"...","body":"..."}')
  Other gmail tools: Gmail_SendEmail, Gmail_ChangeEmailLabels, Gmail_TrashEmail.
...
{_care_instructions(request, context)}
...
def _tool_executor(name: str, args: dict, care_mode: bool = False) -> str:
    ...
    if care_mode and lname in {"schedule_worker", "set_timer"}:
        return ("BLOCKED FOR CARE: use update_care_plan with a routine_event; "
                "the care manager will create and verify its worker.")
...
def _progress_reporter():
    ...
    pretty = {
        ...
        "get_care_plan": "Reading care plan", "update_care_plan": "Updating care",
        "get_care_schedule_status": "Verifying care",
        "heart_rate_measurement": "Measuring pulse",
...
async def run_complex_query(request: str, context: str = "") -> str:
    ...
    For care turns and scheduled care events, the returned string goes directly
    to streaming TTS. Ordinary complex-query results still pass through Kiki's
    local speaking model, so every result must remain short and speakable.
    """
    ...
    active_session = _active_care_session()
    care_mode = is_care_request(request, context) or bool(active_session)
    deadline = float(
        cfg["health_measurement_deadline_seconds"]
        if care_mode and _is_heart_rate_flow(request, context, active_session)
        else cfg["deadline_seconds"])
    ...
    started = time.time()
    unverified = False   # answered without a tool call (care final-turn escape)
    ...
    try:
        ...
        def executor(name: str, args: dict) -> str:
            # Keep the ordinary two-argument path stable for existing callers
            # and tests; the extra policy flag exists only for care runs.
            output = (_tool_executor(name, args, care_mode=True) if care_mode
                      else _tool_executor(name, args))
            if (care_mode and name == "update_care_plan"
                    and str(output).startswith("SUCCESS:")):
                marker = "RECEIPT_JSON:"
                if marker in str(output):
                    try:
                        metadata = json.loads(str(output).split(marker, 1)[1].strip())
                    except (TypeError, ValueError, json.JSONDecodeError):
                        metadata = {}
                    if (metadata.get("section") == "routine_event"
                            and metadata.get("item_id")):
                        care_write_ids.append(str(metadata["item_id"]))
            return output
        ...
        ok, result, _speak, _final, tools_used = await run_agent_loop(
            ...
            # A care turn's verification tool is get_care_plan, never
            # search_web. Naming the wrong one is why the nudge never landed.
            verification_tools=(_CARE_VERIFY_TOOLS if care_mode
                                else _GENERAL_VERIFY_TOOLS),
            # A care answer is spoken straight to the person. Losing the whole
            # turn to "that care action did not complete" is worse than
            # answering from the context the agent was already given, so on the
            # final turn a QUESTION's answer survives. Writes, guided session
            # turns and scheduled routines still fail loudly: an unperformed
            # action reported as done is the one outcome worth a lost turn.
            allow_unverified_finish=(
                care_mode and not active_session
                and is_read_only_care_request(request, context)),
        )
        unverified = bool(isinstance(_final, dict) and _final.get("unverified"))
        if unverified:
            print("[ActionAgent] ⚠ Answer produced without a tool call — "
                  "reporting it, but it is not a verified action.")

        # Verification is a transaction boundary, not merely a prompt wish.
        # If the agent forgot the read-back call, do it here; if the receipt is
        # not active, override any optimistic summary with an honest failure.
        if ok and care_write_ids:
            receipt = _tool_executor(
                "get_care_schedule_status", {"item_id": care_write_ids[-1]},
                care_mode=True)
            try:
                receipt_data = json.loads(receipt)
            except Exception:
                receipt_data = {}
            if not receipt_data.get("scheduled"):
                ok = False
                result = ("The routine was saved but its worker verification failed: "
                          f"{receipt}. Tell the user it is not active yet.")
            else:
                if "get_care_schedule_status" not in tools_used:
                    tools_used.append("get_care_schedule_status")
                workers = receipt_data.get("workers") or []
                next_at = workers[0].get("next_trigger_at") if workers else None
                if next_at and next_at not in str(result):
                    try:
                        readable = datetime.fromisoformat(next_at).strftime(
                            "%d %b, %I:%M %p")
                    except (TypeError, ValueError):
                        readable = str(next_at)
                    if re.search(r"[\u0900-\u097f]", request):
                        suffix = f" अगला सत्र {readable} पर सत्यापित है।"
                    else:
                        suffix = f" The next session is verified for {readable}."
                    result = str(result).rstrip() + suffix
    except Exception as exc:
    ...
    recorder.end_session(
        sid, status=("unverified" if ok and unverified else
    ...
    # never happened.
    if care_mode:
        hindi = bool(re.search(r"[\u0900-\u097f]", request))
        if timed_out:
            spoken = ("[reassuring] यह देखभाल वाला काम समय पर पूरा नहीं हो पाया। "
                    "मैं इसे पूरा हुआ नहीं मानूँगी; हम दोबारा कोशिश कर सकते हैं।"
                    if hindi else
                    "[reassuring] I could not finish that care action in time. "
                    "I will not treat it as completed; we can try again.")
            return "CARE_ACTION_FAILED: " + spoken
        # An agent that stopped to ASK something has not failed vaguely -- it
        # knows exactly what it needs. "There are two people called Suyash;
        # which one?" is the whole value of the turn, and a question can never
        # be mistaken for a fabricated success, which is the only thing the
        # canned line exists to prevent. Same reasoning as
        # allow_unverified_finish above: losing a real answer costs more than
        # the wording it replaces.
        if "?" in summary and _is_meaningful(summary):
            print("[ActionAgent] care turn is asking a question — speaking it "
                  "rather than the generic failure line")
            return "CARE_ACTION_FAILED: [reassuring] " + summary
        spoken = ("[reassuring] यह देखभाल वाला काम पूरा नहीं हुआ। "
                "मैं इसे सफल नहीं बताऊँगी; हम जानकारी जाँचकर फिर कोशिश करेंगे।"
                if hindi else
                "[reassuring] That care action did not complete. I will not report "
                "it as successful; we can check the details and try again.")
        return "CARE_ACTION_FAILED: " + spoken
    if timed_out:
