...
# path). Auth matches to_do/orphan_scripts/apiusage.py: the service-account key file is loaded and
...
_VERTEX_SA_FILE = os.getenv(
    ...
    str(Path(__file__).resolve().parents[2] / "tools_and_config" / "vertex-service-account.json"),
)
...
def _get_vertex_genai_client():
    ...
    Mirrors to_do/orphan_scripts/apiusage.py exactly: service_account.Credentials.from_service_account_file
...
def _get_vertex_credentials():
    """Lazily load the service-account key json used for Vertex auth (to_do/orphan_scripts/apiusage.py)."""
...
def _get_local_client(cfg):
    ...
    default_base = get_llm_config().get("local_api_base", "http://100.64.0.10:8080/v1")
    return OpenAI(
...
def _call_gemini(content, b64_image=None, thinking_level="MEDIUM", websearch=False, model="gemini-3-flash-preview"):
    """Call Gemini on Vertex AI (service-account auth, like to_do/orphan_scripts/apiusage.py).
