...
# --- Vertex AI auth (matches to_do/orphan_scripts/apiusage.py) ---
...
# auth to_do/orphan_scripts/apiusage.py performs with service_account.Credentials.from_service_account_file.
_VERTEX_SA_FILE = os.getenv(
    "VERTEX_SERVICE_ACCOUNT_FILE",
    str(Path(__file__).resolve().parents[1] / "tools_and_config" / "vertex-service-account.json"),
)
_VERTEX_PROJECT = os.getenv("VERTEX_PROJECT", "")
_VERTEX_LOCATION = os.getenv("VERTEX_LOCATION", "global")  # gemini-3.x preview models are served global-only

# Load the service-account JSON key once at import, if there is one. The key file
# is never committed, so on a fresh checkout these stay None: the Vertex fallback
# is then unavailable and everything else, including the whole test suite, still
# imports. Set VERTEX_SERVICE_ACCOUNT_FILE and VERTEX_PROJECT to enable it.
vertex_credentials = None
vertex_credentials_json = None
if os.path.exists(_VERTEX_SA_FILE):
...
_LOCAL_API_BASE = _LLM_CFG.get("local_api_base", "http://100.64.0.10:8080/v1").rstrip("/")
...
def _get_tools_instruction():
    ...
    try:
        ...
        care_mode = mode_has_capability("care")
    except Exception:
        care_mode = False
    ...
    local_hints = {
        ...
        "update_care_plan": ("Senior mode: edit care plan. data MUST be an object. "
                             "A reminder needs message + schedule; ask for time first if missing."),
        "alert_family": "Senior mode: email family on distress/emergency/medical need.",
        "start_care_session": ("Start requested exercise or company-when-bored "
                               "session NOW. Output ONLY this; Kiki takes over."),
        "complex_query": ("WhatsApp, email, Notion, reminders, files, audio, or "
                          "multi-step work. Pass the request verbatim; output ONLY this."),
    }
    if care_mode:
        local_hints["complex_query"] = (
            "WhatsApp, email, Notion, files, audio, care-plan, or multi-step "
            "work. Pass verbatim; output ONLY this.")
    ...
    if "recall_memory" in available:
        instruction += (
            "MANDATORY: Past conversations/memories/what someone said = "
            "`recall_memory`. Never guess.\n"
            "`recall_memory` searches chats, knowledge and research\n"
        )
        if "complex_query" in available:
            instruction += (
                "VERIFIED CURRENT CONTEXT is known: answer it. For current "
                "WhatsApp/email/Notion use `complex_query`, not `recall_memory`.\n"
            )
    ...
    for tool in main_tools:
        ...
        #
        # Do NOT append "?" to optional names. The live Gemma copied
        # `start_care_session(routine?)` literally into JSON as `{"routine?":
        # "Surya Namaskar"}` twice on 2026-08-29, so validation rejected the
        # call before the care-agent handoff could happen. Optionality is less
        # important here than teaching the exact valid JSON key.
        signature = ", ".join(
            key + (f":{_SHORT_TYPES[t]}"
    ...
    if "complex_query" in set(main_names):
        ...
        if care_mode:
            instruction += (
                "ONLY `complex_query` may read, create, edit, or conduct the "
                "daily care plan; do not invent direct care-tool calls.\n"
            )
    ...
    instruction += (
        ...
        "Never say an action was completed or saved before its tool returns success.\n"
...
def _direct_sourced_context_reply(messages, verify_prefill=True):
    """Use a fresh Idle-Mind finding without asking the model to rediscover it.

    The note was already selected and authored by the cloud Unified Idle Mind.
    Letting the small speaking model decide whether to use it a second time was
    both redundant and unreliable: with the exact live WhatsApp note in context,
    the generic tool protocol emitted `recall_memory` or denied access.  This
    narrow read-only route is deterministic; actions, refreshes, stale notes and
    unrelated questions return None and continue through normal tool routing.
    """
    if not verify_prefill:
        return None
    try:
        from core.brain.unified_idle_mind import sourced_note_reply
        return sourced_note_reply(messages)
    except Exception as exc:
        print(f"[IdleMind] sourced-context route skipped: {exc}")
        return None
...
_CARE_ACTION_RE = re.compile(
    r"\b(?:care\s*plan|daily\s*routine|routine|remind|reminder|schedule|add|"
    r"create|change|edit|remove|delete|every\s+day|daily|at\s+\d{1,2})\b|"
    r"(?:केयर\s*प्लान|दिनचर्या|रूटीन|याद\s*दिला|रिमाइंडर|रोज़|रोज|हर\s*दिन|"
    r"बजे|समय|जोड़|बदल|हटा|करना\s*होता)",
    re.IGNORECASE,
)

_CARE_READ_RE = re.compile(
    r"\b(?:what|when|which|show|tell|list|read|today|tomorrow)\b|"
    r"(?:क्या|कब|कौन|कौनसा|बताओ|दिखाओ|पढ़ो|आज|कल)",
    re.IGNORECASE,
)

# A natural clarification often splits the operation and its surface across
# turns: "what's going on?" -> "I mean in my WhatsApp".  The old router tested
# only the final utterance, so neither half contained both regex requirements.
# Keep this deliberately bounded to an explicit clarification and the two most
# recent previous user turns; concatenating arbitrary history would route an
# unrelated mention of WhatsApp because of a question asked minutes earlier.
_AGENT_CLARIFICATION_RE = re.compile(
    r"^(?:yes\s+but\s+)?(?:i\s+mean|no[, ]+i\s+mean|in\s+my|on\s+my|"
    r"from\s+my|there\b|that\s+(?:chat|group|inbox)\b)",
    re.IGNORECASE,
)
_AGENT_REFERENTIAL_RE = re.compile(
    r"\b(?:there|that\s+(?:chat|group|inbox)|those\s+messages|in\s+it)\b",
    re.IGNORECASE,
)


def _previous_user_texts(messages, current: str, limit: int = 2):
    rows = []
    for message in messages or []:
        ...
        if content:
            rows.append(content)
    if rows and rows[-1] == str(current or "").strip():
        rows.pop()
    return rows[-limit:]


def _resolved_complex_request(messages):
    """Current request, or two verbatim user fragments joined as clarification."""
    current = _last_user_text(messages)
    if not current:
        return ""
    previous = _previous_user_texts(messages, current)
    current_surface = bool(_AGENT_SURFACE_RE.search(current))
    current_action = bool(_AGENT_ACTION_RE.search(current))

    if (current_surface and not current_action
            and _AGENT_CLARIFICATION_RE.search(current)):
        for prior in reversed(previous):
            if _AGENT_ACTION_RE.search(prior):
                return f"{prior}\nUser clarification: {current}"

    if (current_action and not current_surface
            and (_AGENT_CLARIFICATION_RE.search(current)
                 or _AGENT_REFERENTIAL_RE.search(current))):
        for prior in reversed(previous):
            if _AGENT_SURFACE_RE.search(prior):
                return f"{prior}\nUser clarification: {current}"
    return current


def _active_care_session():
    try:
...
def _care_followup_context(messages) -> bool:
    recent = messages[-5:] if messages else []
    combined = "\n".join(str(item.get("content", "")) for item in recent)
    if "?" not in combined and not any(token in combined for token in (
            "बताइए", "सुबह", "शाम", "which", "what time", "AM", "PM")):
        return False
    try:
        from core.brain.action_agent import is_care_request
        return is_care_request(combined)
    except Exception:
        return False


def _should_route_complex_query(text, messages=None):
    ...
    text = str(text or "").strip()
    if len(text) < 8:
        # Short replies such as "शाम को" or "हाँ" still belong to a
        # persisted/just-asked care session.
        return bool(_active_care_session() or _care_followup_context(messages))
    try:
        from core.brain.action_agent import is_care_request
        from core.runtime_controls import mode_has_capability
        care_scope = (mode_has_capability("care")
                      or bool(re.search(r"care\s*plan|केयर\s*प्लान", text, re.I)))
        if (care_scope and is_care_request(text)
                and (_CARE_ACTION_RE.search(text) or _CARE_READ_RE.search(text)
                     or _active_care_session()
                     or _care_followup_context(messages))):
            return True
    except Exception:
        pass
    # Resolve only an explicit, adjacent clarification.  Care routing above
    # still evaluates the current utterance because it has its own persisted
    # session/follow-up semantics.
    external_text = _resolved_complex_request(messages) if messages else text
    ...
    if _AGENT_NEGATIVE_RE.search(external_text):
        return False
    if _AGENT_SEND_TO_RE.search(external_text):
        return True
    if not _AGENT_SURFACE_RE.search(external_text):
        return False
    return bool(_AGENT_ACTION_RE.search(external_text))
...
def _auto_complex_query_tool_event(messages, verify_prefill, use_fallback):
    ...
    if (not verify_prefill or use_fallback or not _SEND_TOOLS
        ...
        return None
    current = _last_user_text(messages)
    if not _should_route_complex_query(current, messages):
        return None
    request = _resolved_complex_request(messages)
...
def _auto_engagement_tool_event(messages, verify_prefill, use_fallback):
    """Return a synthetic `start_care_session` turn for a plain "I'm bored".

    The PRIMARY route is the speaking model choosing `start_care_session`
    itself -- its tool hint now covers wanting company, not just exercise. This
    is the backstop for when it does not, in exactly the shape
    `_auto_complex_query_tool_event` uses and for the same reason: the local box
    is small, and a request the model fails to route becomes an ordinary
    sympathetic reply that quietly does nothing.

    Routing it as a TOOL CALL rather than starting the session directly is what
    makes it safe. `is_successful_care_session_handoff` suppresses the follow-up
    local generation on this exact tool, so the care agent takes the microphone
    cleanly; a session started outside that path leaves the speaking model
    talking over the session it just opened.
    """
    if (not verify_prefill or use_fallback or not _SEND_TOOLS
            or "start_care_session" not in _effective_main_tools()[0]):
        return None
    try:
        from core.runtime_controls import mode_has_capability
        if not mode_has_capability("companion"):
            return None
        from core.health.companion_routines import is_engagement_request
        if not is_engagement_request(_last_user_text(messages)):
            return None
        # Never interrupt a session to start one.
        from core.senior.care_plan import get_care_plan_store
        if get_care_plan_store().care_session_state().get("status") == "active":
            return None
    except Exception:
        return None

    arguments = json.dumps({"routine": "Something to think about"},
                           ensure_ascii=False)
    raw = ('<tool_call>{"name": "start_care_session", "arguments": '
           + arguments + "}</tool_call>")
    call = {
        "id": f"call_engage_{int(time.time())}",
...
def _stream_cerebras_speaking(messages, verify_prefill=True, abort_event=None,
    ...
    COST NOTE — images are NEVER sent through this ordinary speaking function.
    `_normalize_messages_for_local`
    ...
    The separate foreground senior-care agent intentionally attaches a fresh
    frame to its own Cerebras request when that event enables continuous vision.
    """
    cfg = _CEREBRAS_CFG
...
def stream_response(messages, use_fallback=False, verify_prefill=True,
    ...
    # A fresh, already-injected finding can answer a read-only status question
    # without a new external lookup. Explicit controls and autobiographical
    # memory requests above retain priority; actions/refreshes are rejected by
    # the sourced-note gate and continue into complex_query below.
    sourced_reply = _direct_sourced_context_reply(
        messages, verify_prefill=verify_prefill)
    if sourced_reply:
        print(f"[IdleMind] Answering from fresh sourced next-turn context: "
              f"{_last_user_text(messages)!r}")
        yield ("sentence", sourced_reply)
        yield ("done", sourced_reply)
        return
    ...
    # "I'm bored" -> the engagement session. Model-first (see the tool hint),
    # this is the backstop.
    auto_engage = _auto_engagement_tool_event(messages, verify_prefill, use_fallback)
    if auto_engage is not None:
        raw, event = auto_engage
        print(f"[Tool] Auto-routed engagement: {_last_user_text(messages)!r}")
        yield ("tool_calls", event)
        yield ("done", raw)
        return
