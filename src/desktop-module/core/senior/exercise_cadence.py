"""Audible timing and cues for guided sessions.
...
def play_cue(name: str, player: str = "mpv") -> bool:
    """Play one short cue. Non-blocking, and never worth failing a turn over.

    Unlike the hold countdown this does NOT block: a cue is under half a second
    and marks a moment that has already happened, so making the person wait for
    it would only add latency to their turn.
    """
    path = cue_track(name)
    if not path:
        return False
    try:
        ...
        print(f"[Cadence] cue {name!r} skipped: {exc}")
        return False
