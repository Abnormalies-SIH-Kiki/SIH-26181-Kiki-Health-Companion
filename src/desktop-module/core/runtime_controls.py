...
def get_active_system_prompt():
    """Return the selected mode prompt, optionally layered on Kiki's default.
    ...
    ``inherit_default_prompt`` is for a capability mode that is still Kiki --
    notably ``health_sih``.  Its health rules are an addendum to the proven
    companion persona, not a replacement persona.  Treating that mode like a
    character replacement removed the detailed curiosity, initiative and
    interest-following rules that made default Kiki feel aware.
    """
    mode = get_active_mode()
    entry = _modes().get(mode, {})
    prompt = entry.get("system_prompt") if isinstance(entry, dict) else None
    default_prompt = get_full_config().get(
        "llm", {}
    ).get("system_prompt", "You are Kiki.")
    inherit_default = bool(
        isinstance(entry, dict) and entry.get("inherit_default_prompt", False))
    if prompt and not inherit_default:
        active_prompt = str(prompt)
    else:
        active_prompt = str(default_prompt)

    addendum = (
        str(entry.get("system_prompt_addendum") or "").strip()
...
# into a briefing about Vaibhav's IMS portal and Goibibo registrations, which
...
def mode_has_own_character():
    ...
    if not isinstance(entry, dict):
        return False
    if entry.get("inherit_default_prompt", False):
        return False
...
def get_persona_reinforcement():
    ...
    if (isinstance(entry, dict)
            and entry.get("inherit_default_prompt", False)):
        # This mode already contains the complete default Kiki persona.  The
        # health addendum is a capability layer, not a replacement character.
        return ""
...
def parse_spoken_control(text):
    ...
    # "mode" ("switch to Amitab", "sound like Vaibav") and pass the exact
