...
class EnvironmentProvider:
    ...
    def _get(self, url: str, params: Dict[str, str]):
        ...
        if url == FORECAST_URL:
            query.update({"daily": "temperature_2m_max,temperature_2m_min,precipitation_probability_max",
                          "forecast_days": 3})
        try:
        ...
        if isinstance(current, dict) and isinstance(payload.get("daily"), dict):
            current = {**current, "_daily": payload["daily"]}
    ...
    def _build_reading(self, weather: Optional[dict], air: Optional[dict]) -> Dict[str, Any]:
        ...
        daily = (weather or {}).get("_daily") or {}
        forecast = []
        for i, day in enumerate(daily.get("time", [])[:3]):
            row = {"date": day}
            for key in ("temperature_2m_max", "temperature_2m_min", "precipitation_probability_max"):
