...
COMPANION_ROUTINES: tuple = (
    ...
    },
    {
        ...
        "category": "memory",
        "default_time": "16:00",
        "objective": "Keep their mind working, using their own life as the "
                     "material. Not a quiz, not a therapy session — the kind of "
                     "conversation that leaves someone feeling sharp.",
        "brief": (
            "This session has no script and no fixed activity, and that is the "
            "point. Invent something worth doing, today, for this person.\n\n"
            "Start by finding out what there is to work with. `conversation_"
            "topics` lists the people, subjects and events they actually have "
            "history with; `recall_memory` gets the detail of whichever one you "
            "pick; `look_at_scene` shows you the room they are sitting in; "
            "`search_web` gets something from the world today. Build the "
            "interaction out of that material, not out of generic content.\n\n"
            "Things that work, as directions rather than a menu: asking about a "
            "half-told story from their past and pushing for the details they "
            "skipped; disagreeing with an opinion they hold and making them "
            "defend it; a guessing round built from their own history; naming "
            "things in a category against the clock; asking them to describe "
            "something in front of them precisely; playing a song and following "
            "wherever it takes them. Combine or ignore these freely.\n\n"
            "Do not call it a game, a quiz, an exercise or an activity, and do "
            "not announce it. Adults do not agree to play games; they get "
            "interested in things. Open with the thing itself.\n\n"
            "Give it shape with sound. `cue` plays a short mark right after you "
            "speak: `start` when a round genuinely begins, `correct`, `wrong` "
            "(a soft descending pair, never a buzzer), `timeup`, `applause` when "
            "they finish something well. Use it to make rounds feel real, and "
            "leave it empty the rest of the time — a sound on every turn stops "
            "meaning anything. Keeping a light score is fine if it makes it more "
            "fun; drop it the moment it stops being.\n\n"
            "Difficulty is a judgement call you make live. Getting things right "
            "is the enjoyable part, so pitch it so they mostly succeed, and make "
            "it harder when they are clearly coasting. If they are struggling, "
            "quietly make it easier rather than announcing that you are. Never "
            "let a wrong answer become a correction — take what was interesting "
            "about it and go there instead.\n\n"
            "Read the room and stop early if they are tired, distracted, or "
            "simply want to talk about something else. Following them somewhere "
            "better is a success, not an abandoned session."
        ),
    },
    {
...
# Deterministic "I would like something to do", so an engagement session can be
# started on demand rather than waited for. Kept narrow: these are statements of
# boredom or a direct request, not any mention of being tired or bored *of*
# something. A session started by mistake is a minor annoyance, but one that
# cannot be started on cue is undemonstrable.
_BORED_RE = re.compile(
    # "bored OF something" is a complaint about that thing, not a request --
    # "I'm bored of this medicine schedule" must not launch an activity.
    r"\b(?:i(?:'m|m| am)?\s+(?:so\s+|really\s+|getting\s+)?bored\b(?!\s+(?:of|with|by))"
    r"|nothing to do|nothing much to do"
    r"|entertain me|cheer me up"
    r"|talk to me about something"
    r"|(?:let'?s|lets)\s+(?:do|play)\s+something)\b",
    re.IGNORECASE,
)
_BORED_HI = (
    "मन नहीं लग", "बोर हो", "बोर लग", "कुछ करने को नहीं",
    "कुछ सुनाओ", "कुछ बताओ न", "टाइम पास",
)


def is_engagement_request(text: str) -> bool:
    """True when the person is asking, in so many words, for something to do."""
    spoken = str(text or "").strip()
    if not spoken:
        return False
    if _BORED_RE.search(spoken):
        return True
    return any(phrase in spoken for phrase in _BORED_HI)
