...
class WorkerDeferred(Exception):
    """The worker could not run *yet*, through no fault of its own.

    Distinct from failure on purpose. A failure burns a retry, records a
    `last_result`, and gets reported into the conversation; a deferral means
    "the moment was wrong, ask again on the next tick" and must do none of
    those things. Raised when a due care routine finds another care session
    already talking to the person — competing for the microphone is not an
    error, but treating it as one exhausted the retry budget, marked the
    worker FAILED, and spammed the chat history with the same failure line
    every five seconds.
    """
...
class Worker:
    ...
    def is_active(self) -> bool:
        ...
        FAILED is deliberately NOT active. Retries do not go through it —
        ``mark_failed`` keeps a worker PENDING while attempts remain and only
        sets FAILED once ``max_retries`` is spent, so FAILED means "gave up".
        Treating it as triggerable resurrected exhausted workers on every
        scheduler tick: two care workers whose ``scheduled_time`` had passed
        reached retry_count 459 against max_retries 3, re-firing every 5 s for
        hours and injecting a failure line into the conversation each time
        until the local model's context overflowed.
        """
        return self.status == WorkerStatus.PENDING.value
