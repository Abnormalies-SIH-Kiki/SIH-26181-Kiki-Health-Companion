...
def test_forecast_is_preserved_and_bounded():
    provider = EnvironmentProvider(CONFIG)
    weather = {**WEATHER, "_daily": {
        "time": ["2026-09-07", "2026-09-08", "2026-09-09", "2026-09-10"],
        "temperature_2m_min": [24], "temperature_2m_max": [31],
        "precipitation_probability_max": [70]}}
    reading = provider._build_reading(weather, AIR)
    assert len(reading["forecast"]) == 3
    assert reading["forecast"][0]["temperature_2m_max"] == 31
    assert reading["forecast"][1]["temperature_2m_max"] is None
