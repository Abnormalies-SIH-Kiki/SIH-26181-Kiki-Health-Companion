...
def test_shipped_health_mode_is_default_kiki_plus_a_compact_health_layer(
        monkeypatch):
    from tools_and_config.config_loader import get_full_config

    cfg = get_full_config()
    shipped = cfg["assistant_modes"]["modes"]["health_sih"]
    monkeypatch.setattr(rc, "get_active_mode", lambda: "health_sih")

    prompt = rc.get_active_system_prompt()
    default = cfg["llm"]["system_prompt"]
    assert shipped["inherit_default_prompt"] is True
    assert prompt.startswith(default)
    assert "HEALTH COMPANION LAYER" in prompt
    assert "proactive, curious Kiki described above" in prompt
    # The layer must stay small enough that restoring Kiki does not evict the
    # conversational history from the local model's 6k-token working budget.
    assert len(shipped["system_prompt_addendum"]) < 1400
    assert rc.mode_has_own_character() is False
