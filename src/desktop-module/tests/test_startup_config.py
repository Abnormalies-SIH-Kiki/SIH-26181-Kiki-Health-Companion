...
def test_no_keeps_config_and_does_not_write(tmp_path):
    ...
    with patch.object(wizard, "_acquire"), \
            ...
            patch.object(wizard, "_wait_clear"), \
...
def test_volume_step_applies_live_and_hold_confirms(tmp_path):
    ...
    # A steadily advancing clock rather than a fixed list: each step is 1.0s, which
    # clears MIN_TAP_S (0.05) for the tap and SELECT_HOLD_S (1.0) for the hold, and
    # does not break when the implementation reads the clock a different number of
    # times.
    clock = itertools.count(0.0, 1.0)

    with patch.object(wizard, "_read", side_effect=readings), \
            patch("core.startup_config.time.sleep", lambda _s: None), \
            patch("core.startup_config.time.monotonic", lambda: next(clock)):
