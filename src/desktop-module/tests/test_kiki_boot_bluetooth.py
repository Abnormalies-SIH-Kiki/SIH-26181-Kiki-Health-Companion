...
def test_find_paired_speaker_name_is_case_insensitive():
    devices = (
        ...
        "Device EF:C0:B9:99:69:9C CLAVIER ONE\n"
    )
    with patch.object(kiki_boot, "_run_quiet", return_value=completed(devices)):
        assert (
            kiki_boot._find_paired_bluetooth_device("Clavier One")
            == "EF:C0:B9:99:69:9C"
        )
...
def test_speaker_name_matches_a_renamed_paired_device():
    # config.json says "Clavier One"; the speaker broadcasts "Clavier Fusion".
    devices = (
        "Device 9C:DE:F0:8D:30:64 OnePlus Nord Buds 3\n"
        "Device EF:C0:B9:99:69:9C Clavier Fusion\n"
    )
    with patch.object(kiki_boot, "_run_quiet", return_value=completed(devices)):
        assert (
            kiki_boot._find_paired_bluetooth_device("Clavier One")
...
def test_failed_connection_unpairs_and_pairs_the_speaker_again():
    ...
    def run_quiet(command, timeout=8):
        ...
        return completed(f"Device {mac} Clavier One\n")
...
def test_absent_speaker_keeps_its_pairing():
    ...
    with (
        patch.object(kiki_boot, "_run_quiet",
                     return_value=completed(f"Device {mac} Clavier One\n")) as run,
...
def test_repair_rediscovers_the_speaker_after_removing_the_bond():
    ...
    def run_quiet(command, timeout=8):
        ...
        listing = f"Device {mac} Clavier Fusion\n" if known["listed"] else ""
    ...
    with (
        ...
        assert kiki_boot._repair_bluetooth_pairing(
            "Clavier One", mac, {"repair_scan_seconds": 12}, answered=True) == mac
...
def test_a_refused_profile_still_counts_as_a_reachable_speaker():
    ...
    mac = "EF:C0:B9:99:69:9C"
    refused = completed(
        "Attempting to connect to EF:C0:B9:99:69:9C\n"
