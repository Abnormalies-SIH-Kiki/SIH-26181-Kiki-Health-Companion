...
# --- what Kiki has actually SEEN today ---

class CountingPlan(FakePlan):
    """A plan that reports observation counts, like the real one now does."""

    def __init__(self, counts=None, **kw):
        super().__init__(**kw)
        self._counts = counts or {}

    def activity_counts(self, since_iso=""):
        return dict(self._counts)


def test_observed_activity_appears_as_counts():
    """The care log had been accumulating vision observations with no reader.

    Counts, never the observations themselves: eight near-identical
    descriptions of someone holding a glass tell a person nothing and cost real
    warm prefix on every later turn.
    """
    line = build_care_now(plan=CountingPlan({"drinking": 3, "exercising": 1}))
    assert "SEEN TODAY drinking x3, exercising x1" in line


def test_nothing_seen_says_nothing():
    assert build_care_now(plan=CountingPlan({})) == ""


def test_only_the_top_three_activities_are_reported():
    line = build_care_now(plan=CountingPlan(
        {"a": 9, "b": 8, "c": 7, "d": 6, "e": 5}))
    assert "d x6" not in line and "e x5" not in line
    assert "a x9" in line and "c x7" in line


def test_a_plan_without_counting_support_is_not_an_error():
    """Older plan objects predate activity_counts entirely."""
    line = build_care_now(plan=FakePlan(measurements=[]))
    assert "SEEN TODAY" not in line


def test_an_unchanged_tally_is_never_re_injected():
    """The anti-obsession rule, at the layer that enforces it.

    A running count of someone's day is exactly the kind of thing a background
    mind will fixate on if it re-enters the prompt every few minutes.
    """
    injector = CareNowInjector(cooldown_seconds=0)
    history = []
    plan = CountingPlan({"drinking": 2})
    for _ in range(10):
        injector.maybe_inject(history, plan=plan)
    assert len(history) == 1
