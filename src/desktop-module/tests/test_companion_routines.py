...
from core.health.companion_routines import is_engagement_request  # noqa: E402


def test_engagement_ships_as_a_routine():
    keys = {r["key"] for r in COMPANION_ROUTINES}
    assert "engagement" in keys


def test_the_engagement_brief_hands_over_material_not_a_menu():
    """The whole point: it names the TOOLS for finding real personal material,
    so the session is built from this person's life rather than a content list."""
    brief = next(r for r in COMPANION_ROUTINES if r["key"] == "engagement")["brief"]
    for tool in ("conversation_topics", "recall_memory", "look_at_scene", "search_web"):
        assert tool in brief, tool


def test_the_engagement_brief_refuses_to_call_it_a_game():
    """Adults do not agree to play games; they get interested in things."""
    brief = next(r for r in COMPANION_ROUTINES if r["key"] == "engagement")["brief"]
    assert "Do not call it a game" in brief
    assert "do not announce it" in brief


def test_the_engagement_brief_asks_for_sound_sparingly():
    brief = next(r for r in COMPANION_ROUTINES if r["key"] == "engagement")["brief"]
    assert "cue" in brief
    assert "leave it empty the rest of the time" in brief


def test_the_engagement_brief_forbids_a_harsh_error_tone():
    brief = next(r for r in COMPANION_ROUTINES if r["key"] == "engagement")["brief"]
    assert "never a buzzer" in brief


@pytest.mark.parametrize("said", [
    "I'm bored", "im so bored", "I am really bored",
    "nothing to do", "entertain me", "cheer me up",
    "lets do something", "let's play something",
    "मन नहीं लग रहा", "बहुत बोर हो रहा हूँ", "कुछ सुनाओ",
])
def test_a_request_for_something_to_do_is_recognised(said):
    assert is_engagement_request(said) is True


@pytest.mark.parametrize("said", [
    "", "I'm tired", "this is boring me", "what time is it",
    "play some music", "I'm bored of this medicine schedule",
    "मुझे नींद आ रही है",
])
def test_ordinary_speech_does_not_start_an_engagement_session(said):
    assert is_engagement_request(said) is False
