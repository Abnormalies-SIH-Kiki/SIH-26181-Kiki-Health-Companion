...
Header:
  - Speak    : a director cue. Type what Kiki should DO ("ask Vaibhav to drink
               water", "give today's WhatsApp updates and the news"); it is
               queued onto main.py's foreground event loop as a `system` message
               (never as a fake user utterance), Kiki performs it out loud in
               character using its normal tools and TTS, and the ordinary
               follow-up window then opens so the room can answer. Needs
               `query_handler` to be wired -- see make_webui_speak_handler in
               main.py.
...
_query_handler = None       # optional callable(text) -> acceptance dict
...
.askbar{display:flex;gap:8px;align-items:center;width:100%;padding-top:2px}
.askbar input{flex:1;min-width:180px;background:#111326;border-color:#3b4070;
padding:9px 12px}.askbar button{padding:9px 18px}.askstate{font-size:12px;color:var(--mut);
min-width:88px}.askstate.ok{color:var(--ok)}.askstate.err{color:var(--err)}
</style></head><body>
<header>
  <h1>🤖 Kiki</h1>
  ...
  <form class="askbar" id="askform">
    <input id="asktext" maxlength="1000" autocomplete="off"
      placeholder="Director cue — e.g. ask Vaibhav to drink water, or read out the WhatsApp updates and news">
    <button class="primary" id="askbutton" type="submit">Speak</button>
    <span class="askstate" id="askstate"></span>
  </form>
</header>
<main>
...
/* ---------------- DIRECTOR CUE ("Speak") ----------------
   The typed line is a stage direction, not a question: Kiki performs it out
   loud in character and then keeps the conversation going. It is queued on the
   same foreground event loop as speech, so pressing Speak mid-sentence lands
   after the current turn rather than talking over it. */
$('#askform').onsubmit=async e=>{
  e.preventDefault();
  const input=$('#asktext'),button=$('#askbutton'),state=$('#askstate');
  const text=input.value.trim();
  if(!text)return;
  button.disabled=true;state.className='askstate';state.textContent='sending…';
  try{
    ...
    if(!response.ok||!result.accepted)throw new Error(result.error||'Kiki is unavailable');
    input.value='';state.className='askstate ok';state.textContent='speaking ✓';
  }catch(err){state.className='askstate err';state.textContent=err.message||'failed';}
  finally{button.disabled=false;input.focus();setTimeout(()=>{state.textContent='';},3500);}
};
...
def _build_app():
    ...
    def manual_query():
        ...
        text = " ".join(str(data.get("query") or "").split()).strip()
        if not text:
            return jsonify({"accepted": False, "error": "Enter an instruction."}), 400
        if len(text) > 1000:
            return jsonify({"accepted": False, "error": "Instruction is too long."}), 400
        if _query_handler is None:
            return jsonify({"accepted": False, "error": "No Kiki device is connected."}), 503
        try:
            result = _query_handler(text)
            if isinstance(result, dict):
                code = 202 if result.get("accepted") else 503
                return jsonify(result), code
            return jsonify({"accepted": bool(result)}), 202 if result else 503
        except Exception as exc:
            return jsonify({"accepted": False, "error": str(exc)}), 503
...
def start_webui(port: int = _PORT, status_provider=None, query_handler=None):
