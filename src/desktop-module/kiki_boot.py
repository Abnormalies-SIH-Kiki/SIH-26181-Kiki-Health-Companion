...
Runs at rpi boot (kikifast.service) and from the settings-menu Restart:
  ...
  7. Ask the laptop server-manager (http://100.64.0.10:8079) to (re)start the three
...
KIKIFAST = str(Path(__file__).resolve().parent)
VENV_PY = "/home/kiki/Kiki/kiki/bin/python"
...
os.environ.setdefault("XAUTHORITY", "/home/kiki/.Xauthority")
...
def _match_device_name(devices, target_name):
    ...
    # name than the one in config.json ("Clavier One" vs "Clavier Fusion").
...
def connect_bluetooth_speaker(settings):
    ...
    name = str(settings.get("name", "Clavier One")).strip()
...
def main():
    ...
    speaker_name = str(speaker_cfg.get("name", "Clavier One"))
