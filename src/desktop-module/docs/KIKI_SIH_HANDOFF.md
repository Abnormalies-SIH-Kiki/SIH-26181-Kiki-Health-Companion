...
A bundled Go bridge holds the WhatsApp session; a Python MCP server exposes it as tools. Kiki reads the real address book from the bridge's own contact store rather than a hand-maintained list, and fuzzy-matches names — *"burrito time"* resolves to the real group *"burgito time"*. On a genuine tie she **asks** instead of messaging the wrong person.
...
| what | where |
|---|---|
| Full architecture reference | `Codestructure.md` |
| Phased plan + status | `plan.md` |
