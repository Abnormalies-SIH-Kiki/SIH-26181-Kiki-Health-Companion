...
def tool_result_note(calls, tool_result):
    ...
    if any(call.get("name") == "complex_query" for call in calls or []):
        return (
            ...
            "outcome. Relay it to Vaibhav conversationally and COMPLETELY — keep "
...
# Every event that opens a full foreground speaking turn (mute -> pause the wake
# word -> model -> TTS -> follow-up window). Named so the routing can be asserted
# in tests: an event missing here is silently ignored by the loop.
TURN_OPENING_EVENTS = (
    "endpoint", "autonomous_vision", "face_wake", "meet_stranger",
    "care_session_start", "care_continue", "webui_instruction",
)


# Foreground handoffs that did NOT come from the microphone. The post-TTS drain
# exists to delete late ASR events; deleting one of these instead would silently
# swallow a scheduled care session, or a dashboard "Speak" cue that was queued
# while the previous turn was still playing.
_PRESERVED_INPUT_EVENTS = ("care_session_start", "webui_instruction")
...
def drain_stale_input_events(event_queue):
    ...
    never got to conduct. Preserve control-plane events in FIFO order.
    """
    preserved = []
    while not event_queue.empty():
        ...
        if (isinstance(item, tuple) and item
                and item[0] in _PRESERVED_INPUT_EVENTS):
...
# The Web UI "Speak" box is a DIRECTOR'S CUE, not a user utterance: the operator
# types what Kiki should do ("ask Vaibhav to drink water", "give the WhatsApp
# updates and the news"), Kiki performs it out loud in character, and then the
# normal follow-up window opens so the room can answer.
#
# It is injected as a `system` message and never appended as `user`. Appending it
# as `user` would be a lie the KV cache then carries for the rest of the
# conversation -- Kiki would "remember" that someone in the room said it, and the
# summarizer would write it down as something the user asked. The wording below
# is deliberately blunt about the two failure modes seen on every prompt of this
# shape: reading the cue aloud verbatim, and prefacing the line with "you asked
# me to tell you...".
WEBUI_SPEECH_CUE = (
    "[DIRECTOR CUE - PERFORM THIS NOW, OUT LOUD, IN CHARACTER]\n"
    "{cue}\n"
    "This cue was TYPED privately by the person operating you. Nobody in the "
    "room said it and nobody heard it. Do not read it out, do not quote it, do "
    "not mention a dashboard, a message, an instruction or being asked to say "
    "anything. Say it as if you thought of it yourself right now, in your own "
    "voice and in the language this conversation is already in, keeping it as "
    "short and natural as real speech. Use your tools first if the cue needs "
    "real information you do not already have. After you speak, stay in the "
    "conversation normally and listen for a reply."
)


def build_directed_speech_instruction(cue):
    """Wrap a dashboard Speak line as the system cue for one spoken turn.

    Returns "" for a blank cue so the caller can refuse the request instead of
    queueing a turn with nothing to perform.
    """
    cue = " ".join(str(cue or "").split()).strip()
    if not cue:
        return ""
    return WEBUI_SPEECH_CUE.format(cue=cue)


def open_directed_speech_turn(message_history, cue):
    """Set up a dashboard-directed turn on ``message_history``.

    Appends the cue as ``system`` and returns it (or "" for a blank cue). It is
    deliberately NOT a ``user`` message: nobody in the room said it, and a fake
    user turn is a lie the KV cache then carries for the rest of the
    conversation -- Kiki would "remember" being asked, and the summarizer would
    write it down as something the person requested. Leaving the turn without a
    user message is exactly what the scheduled-care and autonomous-vision events
    already do.
    """
    instruction = build_directed_speech_instruction(cue)
    if instruction:
        message_history.append({"role": "system", "content": instruction})
    return instruction


def make_webui_speak_handler(loop, event_queue):
    """Build the dashboard's "Speak" handler over one loop + foreground queue.

    Returns ``callable(text) -> acceptance dict``, which `webui/server.py` calls
    on Flask's thread. It only hands the cue to the asyncio loop and returns --
    the turn itself runs on the ordinary lifecycle. Queued rather than spoken
    on the spot, so pressing Speak mid-sentence lands after the current turn
    instead of talking over it.
    """
    def webui_speak_request(cue):
        text = " ".join(str(cue or "").split()).strip()
        if not text:
            return {"accepted": False, "error": "Enter something for Kiki to say."}
        try:
            loop.call_soon_threadsafe(
                event_queue.put_nowait, ("webui_instruction", text))
        except RuntimeError as exc:          # loop already closed / shutting down
            return {"accepted": False, "error": f"Kiki is not running: {exc}"}
        try:
            get_recorder().record("turn", name="webui_speak", phase="queued",
                                  cue=text[:300])
        except Exception:
            pass
        print(f"[WebUI Speak] Queued directed cue: {text[:160]!r}")
        return {"accepted": True, "queued": text}

    return webui_speak_request
...
# One warning per distinct observability failure. The Web UI's live
# context feed is only as trustworthy as this block staying alive.
_observability_warned: set = set()
...
def set_neck_active(state: bool):
    ...
    host = get_full_config().get("controller", {}).get("host", "192.168.1.11")
...
async def main():
    ...
    # are ~10k chars of Vaibhav's life, and injected verbatim they turn "you are
        ...
        try:
            ...
            if _mgr is not None:
                if mode_has_capability("care"):
                    # Seed before activating, exactly as at boot. Without this a
                    # spoken "switch to health mode" activated the care workers
                    # but never installed the companion routines, so the mode
                    # came up half-configured and only a restart fixed it.
                    # Idempotent, so re-entering the mode adds nothing.
    ...
    def mute_stt():
        ...
        # A care session that asked a question and got a whole window of
        # silence is over: the person walked away, or is done. Leaving it
        # active meant everything they said next — minutes later, about
        # anything — went to the care agent instead of the conversation.
        # Saying nothing is therefore a real, third way out.
        try:
            from core.senior.care_voice_agent import end_active_care_session
            end_active_care_session("no reply during the listening window")
        except Exception as _care_exit_err:
            print(f"[Care] silent-window exit failed: {_care_exit_err}")
    ...
    try:
        ...
        # Close anything the previous process left open BEFORE the first turn
        # can be routed. A session that survived a restart used to capture every
        # following utterance -- a maths question answered by the care agent --
        # and each stolen turn refreshed its idle clock, so it could never
        # time out on its own.
        get_care_plan_store().end_orphaned_session()
        _queue_care_session = (
    ...
    # --- Automatic memory recall: build both indexes off the first turn ---
    # The memory index alone is 0.54s cold over ~3.4k records; paying that
    # inside the first question of the day is exactly the stall this feature
    # exists to avoid creating.
    try:
        from core.brain.auto_recall import get_auto_recall
        _recall = get_auto_recall(full_config.get("auto_recall", {}))
        threading.Thread(target=_recall.warm, daemon=True,
                         name="AutoRecallWarm").start()
    except Exception as _recall_err:
        print(f"[AutoRecall] warm-up skipped: {_recall_err}")
    ...
    try:
        ...
        }, query_handler=make_webui_speak_handler(loop, stt_queue))
    except Exception as _e:
    ...
    def ir_return_to_idle():
        ...
        # The physical escape hatch has to end a care session too, or the very
        # next utterance is captured by the agent the person just walked away
        # from.
        try:
            from core.senior.care_voice_agent import end_active_care_session
            end_active_care_session("double-tap returned Kiki to idle")
        except Exception as _care_exit_err:
            print(f"[Care] double-tap exit failed: {_care_exit_err}")
    ...
    try:
        while True:
            ...
            if event == "final":
            ...
            elif event in TURN_OPENING_EVENTS:
                ...
                if (event == "endpoint"
                    ...
                    # "shut up" is the most emphatic exit in the whole system,
                    # and it used to be handled entirely here — so a live care
                    # session never saw it and simply carried on talking.
                    try:
                        from core.senior.care_voice_agent import (
                            end_active_care_session)
                        end_active_care_session("the person said shut up")
                    except Exception as _care_exit_err:
                        print(f"[Care] shut-up exit failed: {_care_exit_err}")
                    if spec_turn is not None:
                ...
                if event == "meet_stranger":
                ...
                elif event == "webui_instruction":
                    # Dashboard "Speak": one directed foreground turn. The cue
                    # goes in as `system` (see WEBUI_SPEECH_CUE) and no `user`
                    # message is appended, because nobody spoke. The rest of the
                    # turn is byte-for-byte the ordinary speaking lifecycle --
                    # same tools, same TTS, same follow-up window -- so Kiki
                    # performs the line and then simply keeps talking.
                    user_utterance = ""
                    collected_sentences = []
                    # A live care session must not swallow this turn: the
                    # operator is directing Kiki, not answering the care agent.
                    guided_care_turn = False
                    open_directed_speech_turn(message_history, text)
                    print(f"\n{'─' * 40}")
                    print(f"[WebUI Speak] Performing directed cue: {text}")
                    print(f"{'─' * 40}")
                    ...
                    # Start the memory lookup the moment the words exist, so it
                    # overlaps the context building below instead of adding to
                    # it. No-op (microseconds) unless the question actually
                    # names something the knowledge base has stored.
                    try:
                        from core.brain.auto_recall import get_auto_recall
                        get_auto_recall().start(user_utterance)
                    except Exception as _recall_err:
                        print(f"[AutoRecall] start skipped: {_recall_err}")

                    # NOTE: "I'm bored" is NOT handled here. It routes through
                    # `start_care_session` as a tool call (the model's own
                    # choice first, `_auto_engagement_tool_event` as backstop),
                    # because `is_successful_care_session_handoff` suppresses the
                    # follow-up local generation on that tool. Starting a session
                    # directly from here skipped that guard and left the speaking
                    # model talking over the session it had just opened.
                    ...
                    # Collect the memory lookup started above. It has had the
                    # whole context-injection block to run in, so this is
                    # normally already finished; if not, it is abandoned rather
                    # than waited on.
                    try:
                        from core.brain.auto_recall import get_auto_recall
                        get_auto_recall().maybe_inject(message_history)
                    except Exception as _recall_err:
                        print(f"[AutoRecall] inject skipped: {_recall_err}")
                ...
                try:
                    ...
                    _last_user = next(
                        (m.get("content") for m in reversed(message_history)
                         if isinstance(m, dict) and m.get("role") == "user"), "")
                    # `messages` is set_context's positional parameter — passing
                    # a count under that name raised TypeError on every single
                    # turn, and the bare `except` below swallowed it. The whole
                    # block died with it: no /api/context snapshot, no turn
                    # session, and `_turn_sid` left None so the ttfw and reply
                    # steps never attached either.
                    _rec.set_context(message_history, count=len(message_history))
                    ...
                    # Never silent again. Bounded by the error text, so a
                    # genuinely broken recorder says so once instead of once
                    # per turn forever.
                    _signature = f"{type(_obs_err).__name__}: {_obs_err}"
                    if _signature not in _observability_warned:
                        _observability_warned.add(_signature)
                        print(f"[Observability] turn snapshot skipped — {_signature}")
                ...
                try:
                    ...
                    if guided_care_turn:
                        ...
                        # The conversation that was already happening decides
                        # the language — a scheduled session opens with nothing
                        # said yet, and the plan's stored "hi" is not evidence.
                        _recent_user_texts = [
                            m.get("content", "") for m in message_history[-24:]
                            if isinstance(m, dict) and m.get("role") == "user"
                        ][-6:]
                        care_reply = await run_care_voice_turn(
                            user_utterance, stop_event=turn_abort_event,
                            recent_texts=_recent_user_texts)
                        ...
                        care_turn_spoke = bool(
                            spoken
                            and queue_voice_ready_text(tts_streamer, spoken))
                ...
                if ir_idle_requested.is_set():
                    ...
                    try:
                        ...
                        # A short cue marks the moment the turn just created —
                        # a round starting, an answer landing. Played before the
                        # hold and never waited on: it is under half a second
                        # and the moment it marks has already happened.
                        _cue = str(_directive.get("cue", "") or "")
                        if _cue:
                            from core.senior.exercise_cadence import play_cue
                            play_cue(_cue)
                    ...
                    if ir_idle_requested.is_set() or turn_abort_event.is_set():
                        ...
                        cancel_mute_timer()
                        return_to_background_listening()
                    elif not care_turn_spoke:
                        # Nothing was actually said this turn — the care reply
                        # was empty, or `queue_voice_ready_text` refused it as
                        # machine output. Both of the branches below assume the
                        # person just HEARD something: one opens a microphone
                        # for the answer to a question they were never asked,
                        # and the other beeps out a hold for an instruction that
                        # was never given. Neither is safe on a silent turn.
                        print("[Care] Nothing was spoken this turn — returning "
                              "to idle rather than listening for the answer to "
                              "an unasked question.")
                        cancel_mute_timer()
                        return_to_background_listening()
                        ...
                        # expose an open microphone between asanas. This covers
                        # both "that was right, here is the next one" and the
                        # first correction of a movement that was wrong — a
                        # single mismatch is fixed out loud mid-routine, not
                        ...
                        print("[Care] Leading on — microphone stays muted; "
